module mpi
    ...
    ! Made up
    integer, parameter :: MPI_MAX_ERROR_STRING = 2048
    ! This number has to stay this way for the logical switch to work
   ...
   contains
    ...
    subroutine mpi_error_string_proc(rcode, lstring, len, ierr)
        use mpi_c_bindings, only: c_mpi_error_string
        integer, intent(in) :: rcode
        character, intent(in) :: lstring
        integer, intent(in) :: len
        integer, optional, intent(out) :: ierr
        call c_mpi_error_string(rcode, lstring, len, ierr)
    end subroutine

    subroutine mpi_abort_proc(comm_f, errorcode, ierror)
        use mpi_c_bindings, only: c_mpi_abort
        integer, intent(in) :: comm_f
        integer, optional, intent(out) :: errorcode
        integer, optional, intent(out) :: ierror
        call c_mpi_abort(comm_f, errorcode, ierror)
    end subroutine
    ...
    subroutine MPI_Bcast_logical(buffer, count, datatype, root, comm, ierror)
        use mpi_c_bindings, only : c_mpi_bcast_logical
        integer, intent(inout) :: buffer(..)
        integer, intent(in) :: count, datatype, root, comm
        integer, optional, intent(out) :: ierror
        call c_mpi_bcast_logical(buffer, count, datatype, root, comm, ierror)
    end subroutine
