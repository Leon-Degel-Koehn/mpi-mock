...
void mpi_abort_wrapper(int *comm_f, int *errorcode, int *ierror) {
    if (*comm_f == MPI_Comm_c2f(MPI_COMM_NULL)) {
        *ierror = MPI_ERR_COMM;
        return;
    }
...
void mpi_error_string(int *rcode, char *lstring, int *len, int *ierr) {
  /* Call C wrapper */
  *ierr = MPI_Error_string(*rcode, lstring, len);

  /* Ensure Fortran string is blank-padded if shorter */
  // if (*len < lstring_len) {
  //   for (int i = *len; i < lstring_len; i++) {
  //     lstring[i] = ' ';
  //   }
  // }
}
...
void mpi_bcast_logical_wrapper(int *buffer, int *count, int *datatype_f, int *root, int *comm_f, int *ierror) {
    MPI_Comm comm = MPI_Comm_f2c(*comm_f);
    MPI_Datatype datatype;
    switch (*datatype_f) {
        case 0:
